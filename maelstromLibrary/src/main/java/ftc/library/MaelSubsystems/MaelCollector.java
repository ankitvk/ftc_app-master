...
public class MaelCollector implements LibConstants, Subsystem {
    ...
    //public MaelMotorSystem collector;
    public MaelMotor oneMotorCollector;
    public MaelMotorSystem twoMotorCollectors;
    public MaelCRServo oneCrCollector;
    public MaelCRServoSystem twoCrCollector;
    ...
    public MaelCollector(String name1, Motor motor, SubsystemModels model, HardwareMap hwMap){
        if(model == SubsystemModels.MOTOR){ oneMotorCollector = new MaelMotor(name1,motor,hwMap);
            this.motor = motor;
            this.model = model;
            numOfMotors = 1;}

        else if(model == SubsystemModels.CR){oneCrCollector = new MaelCRServo(name1,hwMap);
            this.motor = motor;
            this.model = model;
            numOfCr = 1;}
    }
    ...
    public MaelCollector(String name1, String name2,
        ...
        if(model == SubsystemModels.MOTOR){
            if(direction1 == Direction.REVERSE) twoMotorCollectors = new MaelMotorSystem(name1,name2, DcMotorSimple.Direction.REVERSE, DcMotorSimple.Direction.FORWARD,hwMap,motor);
            else if(direction2 == Direction.REVERSE) twoMotorCollectors = new MaelMotorSystem(name1, name2, DcMotorSimple.Direction.FORWARD, DcMotorSimple.Direction.REVERSE, hwMap, motor);
            this.motor = motor;
            this.model = model;
            numOfMotors = 2;
        }
        else if(model == SubsystemModels.CR){
            if(direction1 == Direction.REVERSE){ twoCrCollector = new MaelCRServoSystem(name1,name2, DcMotorSimple.Direction.REVERSE, DcMotorSimple.Direction.FORWARD, hwMap);}
            else if(direction2 == Direction.REVERSE) twoCrCollector = new MaelCRServoSystem(name1,name2, DcMotorSimple.Direction.FORWARD, DcMotorSimple.Direction.REVERSE,hwMap);
            this.motor = motor;
            this.model = model;
            numOfCr = 2;
        }
    ...
    public void DriverControl(MaelController controller, MaelController.Toggler toggler) {

        if(toggler == MaelController.Toggler.OFF){
            if(controller.y()) intake();
            else if(controller.x()) outtake();
            else stop();
        }
        else if(toggler == MaelController.Toggler.ON){
